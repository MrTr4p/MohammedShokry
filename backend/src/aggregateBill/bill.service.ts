...
async function findAlll(bill) {
  ...
  for (let i = 0; i < workersArray.length; i++) {
    ...
    for (let l = 0; l < workers.length; l++) {
      if(workers[i].work == workersArray[l]){
        workersCost = workersCost + workers[i].salary + workers[i].salaryPrecentage
      }
    }
  ...
  const {totalCost, precentage} = await getTotalCost(finalExpensesBill, finalWorkersBill , bill.officePrecentage)
